...
本ツールで暗号化されたファイル (`.nkct`) および署名ファイル (`.nkcs`) は、異なる言語（C++/Rust）やバックエンド（OpenSSL/wolfSSL/RustCrypto）間での完全な相互運用性を確保するため、以下の **Version 1 統一ヘッダー形式** を採用しています。

### **共通バイナリレイアウト (Version 1)**
