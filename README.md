...
**暗号初心者の方や、まずは簡単に試してみたい方は、[GETTING\_STARTED.md](GETTING_STARTED.md) をご覧ください。**
...
このプログラムの心臓部は、**非同期I/Oライブラリ Asio** を利用して構築されたパイプラインです。以下の3つのステージが、キューを介して連携し、それぞれが独立して並列に動作します。
...
```mermaid
graph TD
    A["Reader"] -->|データチャンクをキューに投入| Q(fa:fa-exchange-alt タスクキュー)
    P1["Processor 1"] -- "仕事を取る" --> Q
    P2["Processor 2"] -- "仕事を取る" --> Q
    P3["Processor ..."] -- "仕事を取る" --> Q
    Q -->|処理済みデータを結果へ| R(fa:fa-exchange-alt 結果マップ)
    W["Writer"] -- "結果を書く" --> R
```
...
1.  **ファイル読み込み (Asynchronous Reading)**
    *   入力ファイルから一定のサイズ（例：64KB）のデータチャンクを非同期で読み込み、即座に中央の「タスクキュー」に投入します。Readerは他のステージの状況を待たず、可能な限り速く仕事を供給し続けます。

2.  **暗号化・復号 (Concurrent Processing)**
    *   複数のProcessorスレッドが、タスクキューを監視しています。キューに仕事（データチャンク）が投入されると、手の空いているProcessorが即座にそれを取得し、暗号化または復号処理を並列で実行します。

3.  **ファイル出力 (Asynchronous Writing)**
    *   処理済みのデータチャンクは、順序情報を伴って「結果マップ」に格納されます。Writerは、このマップから正しい順序のブロックを取り出し、出力ファイルに非同期で書き込みます。
...
*   **マルチスレッド (Multithread) - Processorステージ**:
    CPU負荷の高い暗号化・復号処理（Processorステージ）は、複数の専用スレッド (`std::thread`) を使用して実行されます。これにより、複数のCPUコアを同時に利用し、データチャンクの処理を真に並列で進めることで、計算能力を最大限に引き出します。
