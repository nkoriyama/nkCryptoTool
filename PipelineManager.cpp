...
void PipelineManager::run(const std::string& in_path, async_file_t out_file, uintmax_t read_offset, uintmax_t read_size,
    ...
    if (ec) { 
        call_completion_handler(ec, "Failed to open input file: " + in_path); 
        return; 
    }
    ...
    asio::co_spawn(io_context_, writer_coroutine(self), [this, self](std::exception_ptr p) {
        if (p) {
            ...
            catch (const std::system_error& e) { call_completion_handler(e.code(), "Writer: " + std::string(e.what())); }
            catch (const std::exception& e) { call_completion_handler(std::make_error_code(std::errc::io_error), "Writer: " + std::string(e.what())); }
            catch (...) { call_completion_handler(std::make_error_code(std::errc::io_error), "Writer: unknown exception"); }
        }
    });
    asio::co_spawn(io_context_, reader_coroutine(self), [this, self](std::exception_ptr p) {
        if (p) {
            try { std::rethrow_exception(p); }
            catch (const std::system_error& e) { call_completion_handler(e.code(), "Reader: " + std::string(e.what())); }
            catch (const std::exception& e) { call_completion_handler(std::make_error_code(std::errc::io_error), "Reader: " + std::string(e.what())); }
            catch (...) { call_completion_handler(std::make_error_code(std::errc::io_error), "Reader: unknown exception"); }
        }
    });
}

asio::awaitable<void> PipelineManager::writer_coroutine(std::shared_ptr<PipelineManager>) {
    try {
        while (true) {
        ...
        call_completion_handler(e.code(), "Writer coroutine: " + std::string(e.what()));
    } catch (const std::exception& e) {
        call_completion_handler(std::make_error_code(std::errc::io_error), "Writer coroutine: " + std::string(e.what()));
    } catch (...) {
        call_completion_handler(std::make_error_code(std::errc::io_error), "Writer coroutine: unknown exception");
    }
}

asio::awaitable<void> PipelineManager::reader_coroutine(std::shared_ptr<PipelineManager>) {
    try {
        while (is_running_) {
            ...
            if (n > 0) {
                if (chunk.size() != n) chunk.resize(n);
                total_read_ += n;
            ...
            if (ec) { 
                call_completion_handler(ec, "Reader: read error"); 
                co_return; 
            }
        ...
        call_completion_handler(e.code(), "Reader coroutine: " + std::string(e.what()));
    } catch (const std::exception& e) {
        call_completion_handler(std::make_error_code(std::errc::io_error), "Reader coroutine: " + std::string(e.what()));
    } catch (...) {
        call_completion_handler(std::make_error_code(std::errc::io_error), "Reader coroutine: unknown exception");
    }
...
void PipelineManager::run_sync(const std::string& in_path, const std::string& out_path, uintmax_t read_offset, uintmax_t read_size) {
    ...
    if (!out_path.empty() && out_path != "/dev/null") {
        output_file.open(out_path, std::ios::binary | std::ios::trunc);
    }
    ...
    while (total_read_sync < read_size) {
        ...
        // デバッグ: 署名対象データを標準エラーにダンプ (最初の10バイトのみ)
        // std::cerr << "[Debug] run_sync data chunk size=" << data.size() << " first_byte=" << (int)data[0] << std::endl;

        for (const auto& stage : stages_) {
            auto out_data = stage(data);
            if (has_output_) data = std::move(out_data);
        }
        
        if (has_output_ && output_file.is_open() && !data.empty()) {
            output_file.write(data.data(), (std::streamsize)data.size());
        }
