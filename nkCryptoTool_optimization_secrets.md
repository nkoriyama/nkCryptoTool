...
### 0. なぜ`PipelineManager`が必要だったのか
`nkCryptoTool` の初期設計では、ファイルを一度にすべてメモリに読み込んでから処理を行う構造を採用していました。この単純な設計は、小さなファイルには有効でしたが、数GBを超える巨大なファイルを扱う際、メモリ不足（OOM）により処理が不可能になるという致命的な課題を抱えていました。
「巨大なファイルを、限られたメモリリソースで、かつ最高速度で処理するにはどうすればよいか？」
この問いへの答えとして、以下の技術的ブレークスルーを導入しました：
1.  **ストリーミング処理への転換**: 全データをメモリに保持せず、小さなチャンク単位で流れるように処理するパイプラインアーキテクチャへの移行。
2.  **非同期I/Oの採用 (Boost.Asio)**: ディスク読み込み・処理・書き込みを完全に分離し、各ステージを並列化することで、I/O待ち時間を隠蔽。
3.  **C++23 コルーチンの導入**: 複雑になりがちな非同期処理のコールバック地獄を排除し、可読性と保守性の高いコードベースを実現。
`PipelineManager` は、この進化の過程で生まれたコアコンポーネントであり、現代的なハードウェア性能を極限まで引き出すためのエンジニアリングの結晶です。
...
2.  **タスクキュー (`task_queue_`)**:
    ...
    *   **注意**: このキューのサイズには明示的な上限が設定されていません。そのため、ファイルの読み込み速度が処理速度を大幅に上回った場合、キューに待機中のタスクが蓄積され、システムの利用可能なメモリ量に依存してメモリ使用量が増加する可能性があります。
...
### 2. C++23 コルーチン (`asio::awaitable`)

*   **非同期処理の簡潔な記述**: C++で標準化されたコルーチン機能（Asioライブラリでは `asio::awaitable` として提供）を全面的に採用しています。これにより、複雑なコールバックチェーンを避け、非同期処理のロジックを同期コードのように直線的かつ読みやすく記述できます。
