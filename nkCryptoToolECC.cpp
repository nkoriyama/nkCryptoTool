...
void nkCryptoToolECC::encryptFileWithSync(
    ...
    try {
        ...
        // 1. Key Derivation (same as pipeline version)

        auto recipient_public_key_res = loadPublicKey(key_paths.at("recipient-pubkey"));

        if (!recipient_public_key_res) throw std::runtime_error("Failed to load recipient public key: " + toString(recipient_public_key_res.error()));
        ...
        auto secret = ecdh_generate_shared_secret(ephemeral_ecdh_key.get(), recipient_public_key.get());
        ...
        // 2. Cipher Context Initialization
        ...
        // 3. File I/O and Synchronous Loop

        std::ifstream input_file(input_filepath, std::ios::binary);

        if (!input_file) throw std::runtime_error("Failed to open input file: " + input_filepath);



        std::ofstream output_file(output_filepath, std::ios::binary | std::ios::trunc);

        if (!output_file) throw std::runtime_error("Failed to open output file: " + output_filepath);
        ...
        while (input_file.read(in_buffer.data(), in_buffer.size())) {
            ...
            if (EVP_EncryptUpdate(ctx.get(), out_buffer.data(), &out_len, reinterpret_cast<const unsigned char*>(in_buffer.data()), bytes_read) <= 0) {

                throw std::runtime_error("Encryption update failed during synchronous processing.");
        ...
        if (bytes_read > 0) {
            ...
            if (EVP_EncryptUpdate(ctx.get(), out_buffer.data(), &out_len, reinterpret_cast<const unsigned char*>(in_buffer.data()), bytes_read) <= 0) {
                ...
                throw std::runtime_error("Encryption update failed on final chunk.");
        ...
        if (EVP_EncryptFinal_ex(ctx.get(), out_buffer.data(), &out_len) <= 0) {

            throw std::runtime_error("Failed to finalize synchronous encryption.");
        ...
        if (EVP_CIPHER_CTX_ctrl(ctx.get(), EVP_CTRL_GCM_GET_TAG, GCM_TAG_LEN, tag.data()) <= 0) {

            throw std::runtime_error("Failed to get GCM tag during synchronous encryption.");
        ...
        std::cerr << "\nSynchronous encryption failed: " << e.what() << std::endl;
...
void nkCryptoToolECC::decryptFileWithSync(
    ...
    try {
        ...
        if (!user_private_key_res) throw std::runtime_error("Failed to load user private key: " + toString(user_private_key_res.error()));
        ...
        auto secret = ecdh_generate_shared_secret(user_private_key.get(), ephemeral_pub_key.get());
        ...
        // 2. Initialize Cipher and Set Tag
        ...
        std::cerr << "\nSynchronous decryption failed: " << e.what() << std::endl;
