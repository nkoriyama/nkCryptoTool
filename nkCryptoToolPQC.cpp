...
namespace { // Anonymous namespace for internal helpers
    std::unique_ptr<EVP_PKEY, EVP_PKEY_Deleter> generate_ephemeral_ec_key() {
        ...
        if (!pctx || EVP_PKEY_keygen_init(pctx.get()) <= 0) return nullptr;
        OSSL_PARAM params[] = {
            OSSL_PARAM_construct_utf8_string("group", (char*)"prime256v1", 0),
            OSSL_PARAM_construct_end()
        };
        if (EVP_PKEY_CTX_set_params(pctx.get(), params) <= 0) return nullptr;
        EVP_PKEY* pkey = nullptr;
        if (EVP_PKEY_keygen(pctx.get(), &pkey) <= 0) return nullptr;
    ...
    std::vector<unsigned char> ecdh_generate_shared_secret(EVP_PKEY* private_key, EVP_PKEY* peer_public_key) {
        ...
        if (!ctx || EVP_PKEY_derive_init(ctx.get()) <= 0 || EVP_PKEY_derive_set_peer(ctx.get(), peer_public_key) <= 0) return {};
        size_t secret_len;
        if (EVP_PKEY_derive(ctx.get(), nullptr, &secret_len) <= 0) return {};
        std::vector<unsigned char> secret(secret_len);
        if (EVP_PKEY_derive(ctx.get(), secret.data(), &secret_len) <= 0) return {};
...
bool nkCryptoToolPQC::generateEncryptionKeyPair(const std::filesystem::path& public_key_path, const std::filesystem::path& private_key_path, const std::string& passphrase) {
    ...
    if (!priv_bio || PEM_write_bio_PKCS8PrivateKey(priv_bio.get(), kem_key.get(), EVP_aes_256_cbc(), nullptr, 0, pem_passwd_cb, (void*)&passphrase) <= 0) { printOpenSSLErrors(); return false; }
...
bool nkCryptoToolPQC::generateSigningKeyPair(const std::filesystem::path& public_key_path, const std::filesystem::path& private_key_path, const std::string& passphrase) {
    ...
    std::unique_ptr<BIO, BIO_Deleter> priv_bio(BIO_new_file(private_key_path.string().c_str(), "wb"));
    if (!priv_bio || PEM_write_bio_PKCS8PrivateKey(priv_bio.get(), dsa_key.get(), EVP_aes_256_cbc(), nullptr, 0, pem_passwd_cb, (void*)&passphrase) <= 0) { printOpenSSLErrors(); return false; }
    std::unique_ptr<BIO, BIO_Deleter> pub_bio(BIO_new_file(public_key_path.string().c_str(), "wb"));
    ...
    CompressionAlgorithm algo, std::function<void(std::error_code)> completion_handler)
{
    ...
    // 2. ECC part (ECDH) - only for hybrid mode
    if (is_hybrid) {
    ...
    header.reserved = is_hybrid ? 1 : 0; // Use reserved field to indicate hybrid
    ...
    std::function<void(std::error_code)> completion_handler)
{
    ...
    // 2. ECC part (ECDH)
    if(is_hybrid) {
        auto recipient_ecdh_private_key = loadPrivateKey(recipient_ecdh_private_key_path);
    ...
    // --- Key Derivation & Start Decryption ---
        ...
        [this, state, input_filepath, pkey = std::move(public_key)](const asio::error_code& read_sig_ec, size_t) mutable {
            ...
            asio::async_read(state->input_file, asio::buffer(state->file_content),
                [this, state, pub_key = std::move(pkey)](const asio::error_code& final_read_ec, size_t) mutable {
                    if (final_read_ec && final_read_ec != asio::error::eof) {
                    ...
                    if (EVP_DigestVerifyInit(mdctx.get(), nullptr, nullptr, nullptr, pub_key.get()) <= 0) {
                        printOpenSSLErrors();
                        state->verification_completion_handler(std::make_error_code(std::errc::io_error), false);
                        return;
                    }
