...
std::unique_ptr<EVP_PKEY, EVP_PKEY_Deleter> generate_ephemeral_ec_key() { 
    ...
    if (!pctx || EVP_PKEY_keygen_init(pctx.get()) <= 0) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to initialize ephemeral EC key generation context."); } 
    OSSL_PARAM params[] = { OSSL_PARAM_construct_utf8_string("group", (char*)"prime256v1", 0), OSSL_PARAM_construct_end() }; 
    if (EVP_PKEY_CTX_set_params(pctx.get(), params) <= 0) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to set ephemeral EC group parameters."); } 
    EVP_PKEY* pkey = nullptr; 
    if (EVP_PKEY_keygen(pctx.get(), &pkey) <= 0) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to generate ephemeral EC key pair."); } 
...
std::vector<unsigned char> ecdh_generate_shared_secret(EVP_PKEY* private_key, EVP_PKEY* peer_public_key) { 
    ...
    if (!ctx || EVP_PKEY_derive_init(ctx.get()) <= 0 || EVP_PKEY_derive_set_peer(ctx.get(), peer_public_key) <= 0) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to initialize ECDH shared secret derivation."); } 
    size_t secret_len; 
    if (EVP_PKEY_derive(ctx.get(), nullptr, &secret_len) <= 0) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to get ECDH shared secret length."); } 
    std::vector<unsigned char> secret(secret_len); 
    if (EVP_PKEY_derive(ctx.get(), secret.data(), &secret_len) <= 0) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to derive ECDH shared secret."); } 
...
bool nkCryptoToolPQC::generateEncryptionKeyPair(const std::filesystem::path& public_key_path, const std::filesystem::path& private_key_path, const std::string& passphrase) {
    ...
    if (!pctx || EVP_PKEY_keygen_init(pctx.get()) <= 0) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to initialize ML-KEM key generation context."); }
    EVP_PKEY* pkey = nullptr;
    if (EVP_PKEY_keygen(pctx.get(), &pkey) <= 0) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to generate ML-KEM key pair."); }
    ...
    if (!priv_bio) { printOpenSSLErrors(); throw std::runtime_error("Error creating private key file: " + private_key_path.string()); }
    bool success = false;
    ...
    if (!success) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to write ML-KEM private key to file."); }
    std::unique_ptr<BIO, BIO_Deleter> pub_bio(BIO_new_file(public_key_path.string().c_str(), "wb"));
    if (!pub_bio || PEM_write_bio_PUBKEY(pub_bio.get(), kem_key.get()) <= 0) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to write ML-KEM public key to file."); }
    return true;
...
bool nkCryptoToolPQC::generateSigningKeyPair(const std::filesystem::path& public_key_path, const std::filesystem::path& private_key_path, const std::string& passphrase) {
    ...
    if (!pctx || EVP_PKEY_keygen_init(pctx.get()) <= 0) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to initialize ML-DSA key generation context."); }
    EVP_PKEY* pkey = nullptr;
    if (EVP_PKEY_keygen(pctx.get(), &pkey) <= 0) { nkCryptoToolBase::printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to generate ML-DSA key pair."); }
    ...
    if (!success) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to write ML-DSA private key to file."); }
    std::unique_ptr<BIO, BIO_Deleter> pub_bio(BIO_new_file(public_key_path.string().c_str(), "wb"));
    if (!pub_bio || PEM_write_bio_PUBKEY(pub_bio.get(), dsa_key.get()) <= 0) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to write ML-DSA public key to file."); }
    return true;
...
void nkCryptoToolPQC::signFile(asio::io_context& io_context, const std::filesystem::path& input_filepath, const std::filesystem::path& signature_filepath, const std::filesystem::path& signing_private_key_path, const std::string&, std::function<void(std::error_code)> completion_handler){
    ...
    asio::async_read(state->input_file, asio::buffer(state->file_content), [this, state, pkey = std::move(private_key)](const asio::error_code& read_ec, size_t) mutable {
        ...
        if (!mdctx) {
            printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to create digest context.");
        }

        // 1. Get the maximum possible signature size directly from the key.
        size_t sig_len = EVP_PKEY_get_size(pkey.get());
        if (sig_len <= 0) {
            printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to get signature size from key.");
        }
        ...
        if (EVP_DigestSignInit(mdctx.get(), nullptr, nullptr, nullptr, pkey.get()) <= 0) {
            printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to initialize digest signing.");
        }
        ...
        if (EVP_DigestSign(mdctx.get(), signature.data(), &sig_len, state->file_content.data(), state->file_content.size()) <= 0) {
            printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to sign data.");
        }
...
void nkCryptoToolPQC::verifySignature(asio::io_context& io_context, const std::filesystem::path& input_filepath, const std::filesystem::path& signature_filepath, const std::filesystem::path& signing_public_key_path, std::function<void(std::error_code, bool)> completion_handler){
    ...
    asio::async_read(state->signature_file, asio::buffer(state->signature), [this, state, input_filepath, pkey = std::move(public_key)](const asio::error_code& read_sig_ec, size_t) mutable {
        ...
        asio::async_read(state->input_file, asio::buffer(state->file_content), [this, state, pub_key = std::move(pkey)](const asio::error_code& final_read_ec, size_t) mutable {
            ...
            std::unique_ptr<EVP_MD_CTX, EVP_MD_CTX_Deleter> mdctx(EVP_MD_CTX_new());
            if (EVP_DigestVerifyInit(mdctx.get(), nullptr, nullptr, nullptr, pub_key.get()) <= 0) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to initialize digest verification."); }
...
void nkCryptoToolPQC::encryptFileWithPipeline(
    ...
    try {
        ...
        if (!recipient_mlkem_public_key) { printOpenSSLErrors(); throw std::runtime_error("Failed to load recipient ML-KEM public key."); }

        std::unique_ptr<EVP_PKEY_CTX, EVP_PKEY_CTX_Deleter> kem_ctx(EVP_PKEY_CTX_new(recipient_mlkem_public_key.get(), nullptr));
        if (!kem_ctx || EVP_PKEY_encapsulate_init(kem_ctx.get(), nullptr) <= 0) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: EVP_PKEY_encapsulate_init failed."); }
        size_t secret_len_mlkem = 0, enc_len_mlkem = 0;
                        if (EVP_PKEY_encapsulate(kem_ctx.get(), nullptr, &enc_len_mlkem, nullptr, &secret_len_mlkem) <= 0) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: EVP_PKEY_encapsulate get length failed."); }
        std::vector<unsigned char> secret_mlkem(secret_len_mlkem);
        encapsulated_key_mlkem.resize(enc_len_mlkem);
        if (EVP_PKEY_encapsulate(kem_ctx.get(), encapsulated_key_mlkem.data(), &enc_len_mlkem, secret_mlkem.data(), &secret_len_mlkem) <= 0) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: EVP_PKEY_encapsulate failed."); }
        combined_secret = secret_mlkem;

        if (is_hybrid) {
            auto recipient_ecdh_public_key = loadPublicKey(key_paths.at("recipient-ecdh-pubkey"));
            if (!recipient_ecdh_public_key) { printOpenSSLErrors(); throw std::runtime_error("Failed to load recipient ECDH public key."); }
            auto ephemeral_ecdh_key = generate_ephemeral_ec_key();
            if (!ephemeral_ecdh_key) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to generate ephemeral ECDH key."); }
            std::vector<unsigned char> secret_ecdh = ecdh_generate_shared_secret(ephemeral_ecdh_key.get(), recipient_ecdh_public_key.get());
            if (secret_ecdh.empty()) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: ECDH shared secret generation failed."); }
        ...
        std::vector<unsigned char> encryption_key = hkdfDerive(combined_secret, 32, salt, "hybrid-pqc-ecc-encryption", "SHA3-256");
        if (encryption_key.empty()) { printOpenSSLErrors(); throw std::runtime_error("Failed to derive encryption key with HKDF."); }
        ...
        PipelineManager::FinalizationFunc finalizer = [this, template_ctx](async_file_t out_final) -> asio::awaitable<void> {
            ...
            int final_len = 0;
            if (EVP_EncryptFinal_ex(template_ctx.get(), final_block->data(), &final_len) <= 0) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to finalize encryption."); }
            final_block->resize(final_len);
            if (EVP_CIPHER_CTX_ctrl(template_ctx.get(), EVP_CTRL_GCM_GET_TAG, GCM_TAG_LEN, tag->data()) <= 0) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to get GCM tag."); }
...
void nkCryptoToolPQC::decryptFileWithPipeline(
    ...
    try {
        ...
        if (ec || memcmp(header.magic, MAGIC, sizeof(MAGIC)) != 0 || header.version != 1) { input_file.close(); printOpenSSLErrors(); throw std::runtime_error("Invalid file header"); }
        bool is_hybrid = header.reserved == 1;
        if (is_hybrid && !key_paths.count("recipient-ecdh-privkey")) { input_file.close(); printOpenSSLErrors(); throw std::runtime_error("Hybrid file requires ECDH private key."); }

        uint32_t len;
        std::vector<unsigned char> encapsulated_key_mlkem, ephemeral_ecdh_pubkey_bytes, salt, iv;
        asio::read(input_file, asio::buffer(&len, sizeof(len)), ec); if(ec) {input_file.close(); printOpenSSLErrors(); throw std::runtime_error("Failed to read ml-kem key length");} encapsulated_key_mlkem.resize(len);
        asio::read(input_file, asio::buffer(encapsulated_key_mlkem), ec); if(ec) {input_file.close(); printOpenSSLErrors(); throw std::runtime_error("Failed to read ml-kem key");}
        if(is_hybrid) {
            asio::read(input_file, asio::buffer(&len, sizeof(len)), ec); if(ec) {input_file.close(); printOpenSSLErrors(); throw std::runtime_error("Failed to read ecdh key length");} ephemeral_ecdh_pubkey_bytes.resize(len);
            asio::read(input_file, asio::buffer(ephemeral_ecdh_pubkey_bytes), ec); if(ec) {input_file.close(); printOpenSSLErrors(); throw std::runtime_error("Failed to read ecdh key");}
        }
        asio::read(input_file, asio::buffer(&len, sizeof(len)), ec); if(ec) {input_file.close(); printOpenSSLErrors(); throw std::runtime_error("Failed to read salt length");} salt.resize(len);
        asio::read(input_file, asio::buffer(salt), ec); if(ec) {input_file.close(); printOpenSSLErrors(); throw std::runtime_error("Failed to read salt");}
        asio::read(input_file, asio::buffer(&len, sizeof(len)), ec); if(ec) {input_file.close(); printOpenSSLErrors(); throw std::runtime_error("Failed to read iv length");} iv.resize(len);
        asio::read(input_file, asio::buffer(iv), ec); if(ec) {input_file.close(); printOpenSSLErrors(); throw std::runtime_error("Failed to read iv");}
        
        uintmax_t header_size = 0;
        header_size = input_file.seek(0, asio::file_base::seek_cur, ec);
        if(ec) {input_file.close(); printOpenSSLErrors(); throw std::system_error(ec);}
        input_file.close(ec);
        ...
        if (!recipient_mlkem_private_key) { printOpenSSLErrors(); throw std::runtime_error("Failed to load user ML-KEM private key."); }
        std::unique_ptr<EVP_PKEY_CTX, EVP_PKEY_CTX_Deleter> kem_ctx(EVP_PKEY_CTX_new(recipient_mlkem_private_key.get(), nullptr));
        if (!kem_ctx || EVP_PKEY_decapsulate_init(kem_ctx.get(), nullptr) <= 0) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: EVP_PKEY_decapsulate_init failed."); }
        size_t secret_len_mlkem = 0;
        if (EVP_PKEY_decapsulate(kem_ctx.get(), nullptr, &secret_len_mlkem, encapsulated_key_mlkem.data(), encapsulated_key_mlkem.size()) <= 0) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: EVP_PKEY_decapsulate get length failed."); }
        std::vector<unsigned char> secret_mlkem(secret_len_mlkem);
        if (EVP_PKEY_decapsulate(kem_ctx.get(), secret_mlkem.data(), &secret_len_mlkem, encapsulated_key_mlkem.data(), encapsulated_key_mlkem.size()) <= 0) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Decapsulation failed. The private key may be incorrect or the data corrupted."); }
        combined_secret = secret_mlkem;
        
        if(is_hybrid) {
            auto recipient_ecdh_private_key = loadPrivateKey(key_paths.at("recipient-ecdh-privkey"), "ECDH private key");
            if (!recipient_ecdh_private_key) { printOpenSSLErrors(); throw std::runtime_error("Failed to load user ECDH private key."); }
            ...
            if(!ephemeral_pub_key) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: Failed to parse ephemeral ECDH key."); }
            std::vector<unsigned char> secret_ecdh = ecdh_generate_shared_secret(recipient_ecdh_private_key.get(), ephemeral_pub_key.get());
            if (secret_ecdh.empty()) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: ECDH shared secret generation failed."); }
            combined_secret.insert(combined_secret.end(), secret_ecdh.begin(), secret_ecdh.end());
        }

        std::vector<unsigned char> decryption_key = hkdfDerive(combined_secret, 32, salt, "hybrid-pqc-ecc-encryption", "SHA3-256");
        if (decryption_key.empty()) { printOpenSSLErrors(); throw std::runtime_error("Failed to derive decryption key."); }
        ...
        PipelineManager::FinalizationFunc finalizer = [this, template_ctx, &io_context, input_filepath](async_file_t out_final) -> asio::awaitable<void> {
            ...
            if (EVP_CIPHER_CTX_ctrl(template_ctx.get(), EVP_CTRL_GCM_SET_TAG, GCM_TAG_LEN, tag->data()) <= 0) { printOpenSSLErrors(); throw std::runtime_error("Failed to set GCM tag."); }
            if (EVP_DecryptFinal_ex(template_ctx.get(), final_block->data(), &final_len) <= 0) { printOpenSSLErrors(); throw std::runtime_error("OpenSSL Error: GCM tag verification failed. File may be corrupted or tampered with."); }
            final_block->resize(final_len);
