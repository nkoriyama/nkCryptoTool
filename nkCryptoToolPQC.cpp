...
void nkCryptoToolPQC::encryptFileHybrid( asio::io_context& io_context, const std::filesystem::path& input_filepath, const std::filesystem::path& output_filepath, const std::filesystem::path& recipient_mlkem_public_key_path, const std::filesystem::path& recipient_ecdh_public_key_path, std::function<void(std::error_code)> completion_handler) {
    ...
    if (!kem_ctx || EVP_PKEY_encapsulate_init(kem_ctx.get(), nullptr) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to initialize ML-KEM encapsulation."); }
    size_t secret_len_mlkem = 0, enc_len_mlkem = 0;
    if (EVP_PKEY_encapsulate(kem_ctx.get(), nullptr, &enc_len_mlkem, nullptr, &secret_len_mlkem) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to get ML-KEM encapsulation lengths."); }
    std::vector<unsigned char> secret_mlkem(secret_len_mlkem);
    encapsulated_key_mlkem.resize(enc_len_mlkem);
    if (EVP_PKEY_encapsulate(kem_ctx.get(), encapsulated_key_mlkem.data(), &enc_len_mlkem, secret_mlkem.data(), &secret_len_mlkem) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to perform ML-KEM encapsulation."); }
    ...
    if (is_hybrid) {
        ...
        if (!ephemeral_ecdh_key) { throw std::runtime_error("OpenSSL Error: Failed to generate ephemeral EC key."); }
        ...
        if (!PEM_write_bio_PUBKEY(pub_bio.get(), ephemeral_ecdh_key.get())) { throw std::runtime_error("OpenSSL Error: Failed to write ephemeral EC public key to BIO."); }
...
void nkCryptoToolPQC::decryptFileHybrid( asio::io_context& io_context, const std::filesystem::path& input_filepath, const std::filesystem::path& output_filepath, const std::filesystem::path& recipient_mlkem_private_key_path, const std::filesystem::path& recipient_ecdh_private_key_path, std::function<void(std::error_code)> completion_handler) {
    ...
    if (!kem_ctx || EVP_PKEY_decapsulate_init(kem_ctx.get(), nullptr) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to initialize ML-KEM decapsulation."); }
    size_t secret_len_mlkem = 0;
    if (EVP_PKEY_decapsulate(kem_ctx.get(), nullptr, &secret_len_mlkem, encapsulated_key_mlkem.data(), encapsulated_key_mlkem.size()) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to get ML-KEM decapsulation secret length."); }
    std::vector<unsigned char> secret_mlkem(secret_len_mlkem);
    if (EVP_PKEY_decapsulate(kem_ctx.get(), secret_mlkem.data(), &secret_len_mlkem, encapsulated_key_mlkem.data(), encapsulated_key_mlkem.size()) <= 0) { throw std::runtime_error("OpenSSL Error: ML-KEM decapsulation failed. The private key may be incorrect or the data corrupted."); }
    combined_secret = secret_mlkem;
    if(is_hybrid) {
        ...
        if(!ephemeral_pub_key) { throw std::runtime_error("OpenSSL Error: Failed to parse ephemeral EC public key."); }
...
asio::awaitable<void> nkCryptoToolPQC::encryptFileParallel(
    ...
    std::unique_ptr<EVP_PKEY_CTX, EVP_PKEY_CTX_Deleter> kem_ctx(EVP_PKEY_CTX_new(recipient_mlkem_public_key.get(), nullptr));
    if (!kem_ctx || EVP_PKEY_encapsulate_init(kem_ctx.get(), nullptr) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to init PQC encapsulation."); }
    
    size_t secret_len_mlkem = 0, enc_len_mlkem = 0;
    if (EVP_PKEY_encapsulate(kem_ctx.get(), nullptr, &enc_len_mlkem, nullptr, &secret_len_mlkem) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to get PQC encapsulation lengths."); }
    
    std::vector<unsigned char> secret_mlkem(secret_len_mlkem);
    std::vector<unsigned char> encapsulated_key_mlkem(enc_len_mlkem);
    if (EVP_PKEY_encapsulate(kem_ctx.get(), encapsulated_key_mlkem.data(), &enc_len_mlkem, secret_mlkem.data(), &secret_len_mlkem) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to perform PQC encapsulation."); }
...
asio::awaitable<void> nkCryptoToolPQC::decryptFileParallel(
    ...
    std::unique_ptr<EVP_PKEY_CTX, EVP_PKEY_CTX_Deleter> kem_ctx(EVP_PKEY_CTX_new(recipient_mlkem_private_key.get(), nullptr));
    if (!kem_ctx || EVP_PKEY_decapsulate_init(kem_ctx.get(), nullptr) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to init PQC decapsulation."); }
    
    size_t secret_len_mlkem = 0;
    if (EVP_PKEY_decapsulate(kem_ctx.get(), nullptr, &secret_len_mlkem, encapsulated_key_mlkem.data(), encapsulated_key_mlkem.size()) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to get PQC decapsulation secret length."); }
    ...
    std::shared_ptr<EVP_CIPHER_CTX> template_ctx(EVP_CIPHER_CTX_new(), EVP_CIPHER_CTX_Deleter());
    if (EVP_DecryptInit_ex(template_ctx.get(), EVP_aes_256_gcm(), nullptr, decryption_key.data(), iv.data()) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to initialize decryption cipher."); }
...
asio::awaitable<void> nkCryptoToolPQC::encryptFileParallelHybrid(
    ...
    std::string input_filepath_str,
    std::string output_filepath_str,
    std::string recipient_mlkem_public_key_path_str,
    std::string recipient_ecdh_public_key_path_str
) {
    const std::filesystem::path input_filepath(input_filepath_str);
    const std::filesystem::path output_filepath(output_filepath_str);
    const std::filesystem::path recipient_mlkem_public_key_path(recipient_mlkem_public_key_path_str);
    const std::filesystem::path recipient_ecdh_public_key_path(recipient_ecdh_public_key_path_str);
    ...
    if (secret_ecdh.empty()) { throw std::runtime_error("OpenSSL Error: ECDH shared secret generation failed."); }
    std::unique_ptr<BIO, BIO_Deleter> pub_bio(BIO_new(BIO_s_mem()));
    if (!PEM_write_bio_PUBKEY(pub_bio.get(), ephemeral_ecdh_key.get())) { throw std::runtime_error("OpenSSL Error: Failed to write ephemeral ECDH key to BIO."); }
    BUF_MEM *bio_buf; BIO_get_mem_ptr(pub_bio.get(), &bio_buf);
    std::vector<unsigned char> ephemeral_ecdh_pubkey_bytes(bio_buf->data, bio_buf->data + bio_buf->length);

    // 3. 共有秘密の結合と鍵導出
    std::vector<unsigned char> combined_secret = secret_mlkem;
    combined_secret.insert(combined_secret.end(), secret_ecdh.begin(), secret_ecdh.end());
    std::vector<unsigned char> salt(16), iv(GCM_IV_LEN); 
    RAND_bytes(salt.data(), salt.size()); 
    RAND_bytes(iv.data(), iv.size());
    ...
    uint32_t len;
    len = encapsulated_key_mlkem.size(); co_await asio::async_write(output_file, asio::buffer(&len, sizeof(len)), asio::use_awaitable);
    co_await asio::async_write(output_file, asio::buffer(encapsulated_key_mlkem), asio::use_awaitable);
    len = ephemeral_ecdh_pubkey_bytes.size(); co_await asio::async_write(output_file, asio::buffer(&len, sizeof(len)), asio::use_awaitable);
    co_await asio::async_write(output_file, asio::buffer(ephemeral_ecdh_pubkey_bytes), asio::use_awaitable);
    len = salt.size(); co_await asio::async_write(output_file, asio::buffer(&len, sizeof(len)), asio::use_awaitable);
    co_await asio::async_write(output_file, asio::buffer(salt), asio::use_awaitable);
    len = iv.size(); co_await asio::async_write(output_file, asio::buffer(&len, sizeof(len)), asio::use_awaitable);
    co_await asio::async_write(output_file, asio::buffer(iv), asio::use_awaitable);
    ...
    for (;; read_sequence++) {
        ...
        std::vector<unsigned char> buffer(READ_CHUNK_SIZE);
        auto [read_ec, bytes_read] = co_await input_file.async_read_some(asio::buffer(buffer), asio::as_tuple(asio::use_awaitable));
        if(read_ec && read_ec != asio::error::eof) { *first_exception = std::make_exception_ptr(std::system_error(read_ec)); break; }
        if (bytes_read > 0) { bytes_processed_so_far += bytes_read; }
        if (bytes_read == 0) break;
        buffer.resize(bytes_read);
        (*tasks_in_flight)++;
        asio::post(worker_context, [this, seq_id = read_sequence, data = std::move(buffer), template_ctx, completed_chunks, map_mutex, writer_strand, tasks_in_flight, first_exception, &output_file, next_sequence_to_write]() mutable {
            ...
            asio::post(writer_strand, [this, completed_chunks, map_mutex, next_sequence_to_write, tasks_in_flight, writer_strand, &output_file, first_exception]() mutable {
                ...
                for (;;) {
                    ...
                    auto data_to_write = std::move(it->second);
                    completed_chunks->erase(it);
                    asio::co_spawn(writer_strand, asio::async_write(output_file, asio::buffer(data_to_write), asio::use_awaitable), [tasks_in_flight, first_exception, map_mutex](std::exception_ptr p, std::size_t) { if(p && !*first_exception) { std::scoped_lock lock(*map_mutex); *first_exception = p; } (*tasks_in_flight)--; });
                    (*next_sequence_to_write)++;
                }
            });
        });
    ...
    while (*tasks_in_flight > 0) { co_await asio::steady_timer(executor, std::chrono::milliseconds(20)).async_wait(asio::use_awaitable); }
    if(*first_exception) { std::rethrow_exception(first_exception->value()); }
    ...
    if (final_len > 0) { co_await asio::async_write(output_file, asio::buffer(final_block.data(), final_len), asio::use_awaitable); }
    std::vector<unsigned char> tag(GCM_TAG_LEN);
    ...
    printProgress(1.0);
    std::cout << "\nParallel Hybrid encryption to '" << output_filepath.string() << "' completed." << std::endl;
    co_return;
}

// 並列ハイブリッド復号
asio::awaitable<void> nkCryptoToolPQC::decryptFileParallelHybrid(
    asio::io_context& worker_context,
    std::string input_filepath_str,
    std::string output_filepath_str,
    std::string recipient_mlkem_private_key_path_str,
    std::string recipient_ecdh_private_key_path_str
) {
    const std::filesystem::path input_filepath(input_filepath_str);
    const std::filesystem::path output_filepath(output_filepath_str);
    const std::filesystem::path recipient_mlkem_private_key_path(recipient_mlkem_private_key_path_str);
    const std::filesystem::path recipient_ecdh_private_key_path(recipient_ecdh_private_key_path_str);
    ...
    co_await asio::async_read(input_file, asio::buffer(&len, sizeof(len)), asio::use_awaitable); encapsulated_key_mlkem.resize(len);
    co_await asio::async_read(input_file, asio::buffer(encapsulated_key_mlkem), asio::use_awaitable);
    co_await asio::async_read(input_file, asio::buffer(&len, sizeof(len)), asio::use_awaitable); ephemeral_ecdh_pubkey_bytes.resize(len);
    co_await asio::async_read(input_file, asio::buffer(ephemeral_ecdh_pubkey_bytes), asio::use_awaitable);
    co_await asio::async_read(input_file, asio::buffer(&len, sizeof(len)), asio::use_awaitable); salt.resize(len);
    co_await asio::async_read(input_file, asio::buffer(salt), asio::use_awaitable);
    co_await asio::async_read(input_file, asio::buffer(&len, sizeof(len)), asio::use_awaitable); iv.resize(len);
    co_await asio::async_read(input_file, asio::buffer(iv), asio::use_awaitable);
    ...
    if (EVP_PKEY_decapsulate(kem_ctx.get(), nullptr, &secret_len_mlkem, encapsulated_key_mlkem.data(), encapsulated_key_mlkem.size()) <= 0) { throw std::runtime_error("OpenSSL Error: Failed to get PQC decapsulation secret length."); }
    std::vector<unsigned char> secret_mlkem(secret_len_mlkem);
    if (EVP_PKEY_decapsulate(kem_ctx.get(), secret_mlkem.data(), &secret_len_mlkem, encapsulated_key_mlkem.data(), encapsulated_key_mlkem.size()) <= 0) { throw std::runtime_error("Decapsulation failed. The private key may be incorrect or the data corrupted."); }
    ...
    auto first_exception = std::make_shared<std::optional<std::exception_ptr>>();
    uint64_t read_sequence = 0;
    uintmax_t header_size = sizeof(FileHeader) + sizeof(uint32_t) + encapsulated_key_mlkem.size() + sizeof(uint32_t) + ephemeral_ecdh_pubkey_bytes.size() + sizeof(uint32_t) + salt.size() + sizeof(uint32_t) + iv.size();
    uintmax_t ciphertext_size = total_input_size - header_size - GCM_TAG_LEN;
    uintmax_t bytes_read_so_far = 0; 
    while(bytes_read_so_far < ciphertext_size) {
        if(*first_exception) break;
        ...
        if(read_ec && read_ec != asio::error::eof) { *first_exception = std::make_exception_ptr(std::system_error(read_ec)); break; }
        if (bytes_read > 0) { bytes_read_so_far += bytes_read; }
        if (bytes_read == 0 && bytes_read_so_far < ciphertext_size) { *first_exception = std::make_exception_ptr(std::runtime_error("File ended prematurely.")); break; }
        buffer.resize(bytes_read);
        (*tasks_in_flight)++;
        asio::post(worker_context, [this, seq_id = read_sequence, data = std::move(buffer), template_ctx, completed_chunks, map_mutex, writer_strand, tasks_in_flight, first_exception, &output_file, next_sequence_to_write]() mutable {
            try {
    ...
    std::vector<unsigned char> final_block(EVP_MAX_BLOCK_LENGTH);
    int final_len = 0;
    if (EVP_DecryptFinal_ex(template_ctx.get(), final_block.data(), &final_len) <= 0) { throw std::runtime_error("GCM tag verification failed. File may be corrupted or tampered with."); }
    if(final_len > 0) { co_await asio::async_write(output_file, asio::buffer(final_block.data(), final_len), asio::use_awaitable); }

    printProgress(1.0);
    std::cout << "\nParallel Hybrid decryption to '" << output_filepath.string() << "' completed." << std::endl;
    co_return;
}
